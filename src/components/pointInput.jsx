import { AbsorbtionDropdown } from "./absorbtionDropdown";
import { CeilingAndWallMaterialSelector } from "./ceilingAndWallMaterialSelector";
import { SdiSelector } from "./sdiSelector";
...
export const PointInput = ({
  ...
  const renderCurrentPoints = () => {
    return topPointMatrix.map((point, i) => {
      return (
        <div key={i} className="flex flex-col gap-3 p-2 h-[90px]">
  ...
  return (
    <div className="secondary-box h-full w-full p-4 overflow-scroll ">
      <div className="flex flex-col gap-3 p-2">
        <h1>Ceiling Height(m)</h1>
        <input
          value={ceilingHeight}
          onChange={(e) => {
      ...
      <CeilingAndWallMaterialSelector
      ...
      />
      <SdiSelector sdi={sdi} setSdi={setSdi} />
      <div className="flex">
        <div className="flex flex-col w-full">
          <div>{renderCurrentPoints()}</div>
        </div>
